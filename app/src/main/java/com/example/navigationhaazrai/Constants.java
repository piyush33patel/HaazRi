package com.example.navigationhaazrai;

public class Constants {
    private static final String ROOT_URL = "https://websiteasofjunetwelve.000webhostapp.com/";
    public static final String URL_VIEW_ATTENDANCE = ROOT_URL + "HaazRi/ViewAttendance.php";
    public static final String URL_MARK_ATTENDANCE = ROOT_URL + "HaazRi/MarkAttendance.php";
    public static final String URL_TIME_TABLE_DAY_WISE = ROOT_URL + "HaazRi/TimeTableDayWise.php";
    public static final String URL_DELETE_ATTENDANCE_TUPLE = ROOT_URL + "HaazRi/DeleteAttendanceTuple.php";
    public static final String URL_VIEW_ATTENDANCE_SUBJECT_WISE = ROOT_URL + "HaazRi/ViewAttendanceSubjectWise.php";
    public static final String URL_NEW_USER = ROOT_URL + "HaazRi/NewUser.php";
    public static final String URL_USER_ACCOUNT = ROOT_URL + "HaazRi/Account.php";
    public static final String URL_DATE_WISE = ROOT_URL + "HaazRi/ActivityDayWise.php";

}
